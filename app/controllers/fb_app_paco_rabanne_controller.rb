...
class FbAppPacoRabanneController < ApplicationController
  ...
  def share
    if params[:nombre].present? and params[:email].present? and params[:rut].present? and params[:celular].present?
        @me_from_database = Participant.create(:facebook_idnumber => "111",:facebook_name => params[:nombre]+" "+params[:apellido], :facebook_email => params[:email], :rut => params[:rut], :phone => params[:celular], :address => params[:direccion], :city => params[:region], :province => params[:comuna], :facebook_gender => params[:sexo])
